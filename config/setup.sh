...
func_package_deps(){
  ...
  || [ "${os}" == "ubuntu" ]; then
    ...
    || [ "${os}" == "parrot" ]; then
      ...
      sudo ${arg} apt-get install -y pycrypto
      if [[ "$?" -ne "0" ]]; then
    ...
    sudo ${arg} apt-get install -y mingw-w64 monodevelop mono-mcs unzip ruby golang wget git \
      python pycrypto python-pefile python-pip ca-certificates python3-pip winbind
    if [[ "$?" -ne "0" ]]; then
  ...
  || [ "${os}" == "rhel" ]; then
    sudo ${arg} dnf -y install mingw64-binutils mingw64-cpp mingw64-gcc mingw64-gcc-c++ mono-tools-monodoc monodoc \
      monodevelop mono-tools mono-core unzip ruby golang wget git python pycrypto python-pefile \
    ...
    sudo pacman -Sy ${arg} --needed mingw-w64-binutils mingw-w64-crt mingw-w64-gcc mingw-w64-headers mingw-w64-winpthreads \
      mono mono-tools mono-addins python2-pip wget unzip ruby python python2 pycrypto gcc-go ca-certificates base-devel python-pip krb5 samba
    if [[ "$?" -ne "0" ]]; then
  ...
  || [ "${os}" == "manjaro" ]; then
    ...
    sudo pacman -Sy ${arg} --needed mono mono-tools mono-addins python2-pip wget unzip ruby python python2 pycrypto gcc-go ca-certificates base-devel python-pip krb5 samba
    if [ $(id -u) -eq 0 ]; then
